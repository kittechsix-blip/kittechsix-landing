...
// Exact official logo paths in each platform's real brand colour (`fill="#.."`
// attributes, not CSS, so the brand-audit hex scan treats these as
// illustration rather than stray UI colour). TikTok's real single-tone is
// flat black, invisible on this dark footer, so — as TikTok's own alternate
// lockup does — the mark is layered three times (cyan + red offset behind a
// white main note) for the recognisable colour treatment on a dark surface.
const SOCIAL_ICONS = {
